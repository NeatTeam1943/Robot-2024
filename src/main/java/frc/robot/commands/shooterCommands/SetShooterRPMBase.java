...
public abstract class SetShooterRPMBase extends Command {
  ...
  public SetShooterRPMBase(Shooter shooter) {
    ...
    m_leftController =
        new PIDController(
            ShooterConstants.kLeftP, ShooterConstants.kLeftI, ShooterConstants.kLeftD);
    m_rightController =
        new PIDController(
            ShooterConstants.kRightP, ShooterConstants.kRightI, ShooterConstants.kRightD);

    m_ffLeft =
        new SimpleMotorFeedforward(
            ShooterConstants.kLeftS, ShooterConstants.kLeftV, ShooterConstants.kLeftA);
    m_ffRight =
        new SimpleMotorFeedforward(
            ShooterConstants.kRightS, ShooterConstants.kRightV, ShooterConstants.kRightA);
  ...
  public void execute() {
    m_shooter.setLeftMotorVoltage(
        m_ffLeft.calculate(m_shooter.getLeftRPM())
            + m_leftController.calculate(m_shooter.getLeftRPM()));

    m_shooter.setRightMotorVoltage(
        m_ffRight.calculate(m_shooter.getRightRPM())
            + m_rightController.calculate(m_shooter.getRightRPM()));
  }
