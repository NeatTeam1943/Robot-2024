...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_commands =
        Map.of(
            ...
            "Intake Note",
                new SequentialCommandGroup(
                    m_dis2Commands.intake(0.6), m_dis2Commands.transportIntake(0, 0)));
  ...
  private void configureBindings() {
    ...
    // m_intake.setMotorSpeed(0), m_intake), new RunCommand(() -> m_transport.setBeltsSpeed(0),
    // m_transport)));
    // m_driverController.y().whileFalse(new RunCommand(() -> m_transport.setBeltsSpeed(0),
    // m_transport));
    ...
    m_mechanismController
        .povDown()
        .whileTrue(new RunCommand(() -> m_pitcher.setAngleMotorsSpeed(1), m_pitcher));
    m_mechanismController
        .povUp()
        .whileTrue(new RunCommand(() -> m_pitcher.setAngleMotorsSpeed(-1), m_pitcher));
    ...
    m_drive.setDefaultCommand(
        new RunCommand(() -> m_drive.driveArcade(m_driverController), m_drive));

    // m_shooter.setDefaultCommand(new RunCommand(() ->
    // m_shooter.setShooterMotorsSpeed(0),m_shooter));
  }
  ...
  public Command getAutonomousCommand() {
    ...
    return new SequentialCommandGroup(
        new Shoot(m_dis2Commands, false),
        new InstantCommand(() -> m_shooter.setShooterMotorsSpeed(0), m_shooter),
        new ReachPitch(m_pitcher, 45),
        new ParallelCommandGroup(
            new InitializeIntakeMode(
                m_pitcher, m_shooter))); // , new PassLine(m_drive),m_dis2Commands.intake(-1)),
    // m_dis2Commands.intake(0));
    // return new SequentialCommandGroup(new Shoot(m_dis2Commands, false), new
    // InitializeIntakeMode(m_pitcher, m_shooter));
    // return new SequentialCommandGroup(new Shoot(m_dis2Commands, false),
    // m_dis2Commands.intake(-1), new InitializeIntakeMode(m_pitcher, m_shooter), new
    // PassLine(m_drive), m_dis2Commands.intake(0));
    // return new SequentialCommandGroup(new Shoot(m_dis2Commands, false), m_dis2Commands.intake(1),
    // new PassLine(m_drive), new InitializeShooterMode(m_pitcher, m_shooter, false));
